 ...
 * Generate initial facility outputs, Load orders, Process orders, Generate order outputs, 
 * Generate final facility outputs.
 * 
 * For each order, the output summarizes the order and its processing solution. 
 * In addition to the order output,it also includes a summary of the state of 
 * all facilities including its current inventory, a list of depleted items 
 * (items completely used up), and booking schedule.
 * 
 * @author Ralph Zhang, Lanny Xu.
 */
public class Main {
    
	public static void main(String[] args) throws NullParamException, DataValidationException {
            
            System.out.println("Output 1: Facility Status Output for all 18"
            		+ " facilities before processing orders"); 
            ...
            System.out.println("Output 3: Facility Status Output for all 18"
            		+ " facilities after processing orders"); 
            FacilityMgr.getInstance().printReport();
            System.out.println("---------------------------------------"
                                + "------------------------------------");
